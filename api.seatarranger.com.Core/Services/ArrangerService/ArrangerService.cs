    ...
    public class ArrangerService : IArrangerService
    {
        public Dictionary<TableEntity, HashSet<PartyEntity>> ArrangeParties(PartyEntity[] partyEntities, TableEntity[] tableEntities)
        {
            ...
            for (int tableIndex = 0; tableIndex < tableEntities.Length; tableIndex++)
            {
                ...
                for (int partyIndex = 0; partyIndex < partyEntities.Length; partyIndex++)
                {
                    var party = partyEntities[partyIndex];
                    ...
                    {
                        if (lastAddedParty.Dislikes != null && lastAddedParty.Dislikes.Contains(party))
                        {
                            continue;
                        }

                        if (party.Dislikes != null && party.Dislikes.Contains(lastAddedParty))
                        {
                            continue;
                        }
                    }
